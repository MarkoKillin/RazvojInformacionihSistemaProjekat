...
<body>
	...
	<sec:authorize access="hasRole('Profesor')">
		<div>
			...
			<a href="/Mudl/profesor/getZadatak?idPredmet=${predmet.idPredmet}"><button
				...
				href="/Mudl/profesor/getResenja?idPredmet=${predmet.idPredmet}"><button
					class="dugmepredmet">Prikaz Resenja Zadataka</button></a> <a
	...
	<sec:authorize access="hasRole('Student')">
		<div>
			<c:if test="${predmet != null}">
				<h2>${predmet.naziv}-SpisakOpcija:</h2>
			</c:if>
			<a href="/Mudl/student/getZadatak?idPredmet=${predmet.idPredmet}"><button
					class="dugmepredmet">Prikaz Zadataka</button></a> <a
				href="/Mudl/student/getMaterijal?idPredmet=${predmet.idPredmet}"><button
					class="dugmepredmet">Prikaz Materijala</button></a> <a
				href="/Mudl/student/getAddIzbor?idPredmet=${predmet.idPredmet}"><button
					class="dugmepredmet">Glasaj na Anketi</button></a> <a
				href="/Mudl/student/getAddPredajZadatak?idPredmet=${predmet.idPredmet}"><button
					class="dugmepredmet">Dodaj Resenje Zadatka</button></a>
			<c:if test="${message != null}">
				<h4>${message }</h4>
			</c:if>
		</div>
